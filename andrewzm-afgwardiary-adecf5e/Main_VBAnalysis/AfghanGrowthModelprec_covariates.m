...
afgPopFile = fopen('afg_pop.txt');
Afg_pop = fscanf(afgPopFile, 'r');
...
afgCityFile = fopen('afg_city.txt');
Afg_cities = fscanf(afgCityFile, 'r');
...
for i = 1:size(S1,1)
    for j = 1:size(S1,2)
        %Dist_to_city(i,j) =
        %min(posdist(S1(1,j),S2(i,1),Filtered_cities(:,1),Filtered_cities(:
        %,2))); %CRC commented out this line
    end
end
...
%Basis decomposition
load('AFGBasis')       % Load chosen set of basis %CRC changed the directory of this file
...
% Options
%options = foptions; %CRC commented out this line
options(14) = 2000;
% options(9) = 1;
options(2) = 0.1;
options(3) = 0.1;
...
for i = 2:Constants.N
    ...
    [xestpost(:,i)] = conjgrad(f,xestprior(:,i)',options,gradf)'; %CRC
    %changed this line from scg
...
for i = Constants.N-1:-1:1
    ...
    mudash = conjgrad(f,xestpost(:,i+1)',options,gradf)'; %CRC changed from scg
...
%CRC added this function from
%https://en.wikipedia.org/wiki/Conjugate_gradient_method
function [x] = conjgrad(A,b,x) %added g
    r=b-A*x;
    p=r;
    rsold=r'*r;
